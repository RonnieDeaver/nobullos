...
- Code-quality rule: before coding, read [CODE_QUALITY.md](./CODE_QUALITY.md); it supplements, never overrides, this file, `docs/DO_NOT_BREAK.md`, canonical runbooks, or subsystem contracts.
- Public API documentation rule: before planning/implementing third-party API/integration work, review current public docs for relevant endpoints/fields/errors/SDK methods and cite findings; training-data recall is not enough.
- Prior-task research rule: before planning, search existing tasks (including MERGED) for related subsystem, integration, feature, file, or bug-class work; incorporate decisions/gotchas and list consulted tasks.
- Task validation: read preflight and complete the non-executing self-check; routine agents perform review-by-inspection and run no test, lint, typecheck, gate, or focused substitute. Routine completion is validated by Replit's built-in completion review, not `npm run gate`; an explicitly requested gate runs through the managed **Long validation** workflow, while nightly/weekly/post-merge lanes remain independent. For an unrelated or platform-owned review rejection, preserve the task diff, use provenance when relevant, do not request a fresh review, and do not launch compensating checks or unrelated repairs; a task-owned finding remains blocking. [TESTING.md](./TESTING.md#bounded-task-validation-policy-owner-approved) is canonical, overriding conflicting plan testing instructions.
- Architecture Governor rule: before PLANNING DB/schema, integration, worker, API/auth, dependency, performance, or test control-plane changes, load `.agents/skills/architecture-governor/SKILL.md`; never alter gates, selectors, baselines, or approved policy/budgets without owner approval.
- Task-sizing rule: any oversize signal (≈3+ clusters, ≈800+ lines/10+ files, giant file, rebuild/restructure, multi-pass verification, or vendor-stub infra) mandates a complete `dependsOn`-ordered epic; preserve full scope, never trimmed; final stage integration/full verification; stop/split on compaction or retries. See [TASK_PREFLIGHT.md § 13](./TASK_PREFLIGHT.md) + [.agents/skills/epic-decomposition/SKILL.md](./.agents/skills/epic-decomposition/SKILL.md).
- Task-file exclusivity rule: before activation, every proposed task must declare its expected repository-relative write set and compare it with unfinished Draft, Active, Ready, Merging, and queued work, including ready-but-unapplied work; unknown or incomplete sets fail closed, and any exact overlap stays unstarted behind a dependency until replanned from current main. See [TASK_PREFLIGHT.md § 0](./TASK_PREFLIGHT.md#0-exclusive-task-files); this governs compliant planners but cannot intercept a task manually started through an external task panel.
## System Architecture
...
- **DB Pool Tenancy Rules**: `api`/`worker`/`probe` pools are non-overlapping tenants; sizes in the Runtime Truth Table above. See [Audit Surface Runbook](./RUNBOOKS.md#audit-surface-runbook-db-pool-tenancy--external-call-audit).
...
- **Native booking, Client Conversations & Notifications**: Conversation Hub (`/comms` clients view); Calendly-style booking tool ([BOOKING_RECURRENCE.md](./BOOKING_RECURRENCE.md)); per-thread global read-state in `thread_read_states` ([TWILIO.md](./TWILIO.md)); per-user inbox + Slack DM forwarding via `notifyUser()` ([NOTIFICATIONS.md](./NOTIFICATIONS.md)).
...
### Core Features
- **Client Management + Reporting**: Client Command Center, CRM Dashboard, Integrations Hub; Deals kanban, tags & segments, deal scoring, stage automation (all under `/admin`); lead lifecycle & Leads view (`/leads`, forward-only) — see [LEADS.md](./LEADS.md); monthly reports, PDF import, custom terminology, edit audit trail.
...
- **Front ingestion, recovery & analytics coverage**: Webhook-first pipeline + historical recovery with backlog drains, gap close/backfill, 100% sweeps, auto-upgrade, adoption floor + console (coverage grain: Runtime Truth Table). See [FRONT_ANALYTICS_COVERAGE.md](./FRONT_ANALYTICS_COVERAGE.md), [FRONT.md](./FRONT.md).
...
- **Maintenance schedulers & prod-action self-heal**: Default-OFF worker-pool schedulers (email cleanup, feedback Slack resend, video resume; orphaned-user heal retired) + self-heal (`enable_prod_action_self_heal`). See [PROD_ACTION_SELF_HEAL.md](./PROD_ACTION_SELF_HEAL.md), [WORKERS_QUEUES_RUNBOOK.md](./WORKERS_QUEUES_RUNBOOK.md).
...
- **Revenue Integrity System (`/ris`)**: Reporting-role QA + Performance + Engagement layers over a per-client/product/location checklist ledger; default-OFF BigQuery auto-pull, degrades to Needs Review/gray, never silent Pass. See [RIS.md](./RIS.md).
...
| Concern | Value |
| --- | --- |
| Database provider | Helium Postgres on `heliumdb` (dev) / Neon Postgres on `neondb`, role `neondb_owner` (prod). History: **G-011** in `audits/G-docs-findings.md`. |
| Postgres version | 16.10 (dev) / 16.12 (prod) |
...
| Per-query observability (prod) | `pg_stat_statements` in `shared_preload_libraries` + installed on `neondb`. Dev mirror is non-functional/unused. See [PG_STAT_STATEMENTS_REGRESSION.md](./PG_STAT_STATEMENTS_REGRESSION.md). |
