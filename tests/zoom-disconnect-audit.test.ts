/* test-registration
{
  "name": "Zoom credential-clear auditing (Task #2007)",
  "regression": true,
  "smoke": true,
...
test-registration */
/**
 * Task #2007 — Zoom credential-clear auditing.
 *
 ...
 * locks the same invariants in for Zoom.
 *
 ...
 * tests/zoom-auth-recovery and the Zoom probe-classification tests; this
 * test focuses on the disconnect audit:
 *
 ...
 *
 * Task #2240 — credential isolation via in-memory override. Zoom stores
 * its credential in the shared `system_settings` table, which the live
 * `Start application` workers re-write in the `public` schema while this
 * test runs. Against the shared dev DB that race re-seeds the access token
 * between the disconnect call and the assertion. We now drive the seeded
 * credential through `__setZoomCredentialStoreOverrideForTests`: a tiny
 ...
 * FK-attribution coverage. Mirrors the Front/SEMrush fix.
 */
...
// Zoom stores its OAuth credential in `system_settings`; the audit
...
async function main(): Promise<void> {
  console.log("Zoom credential-clear auditing (Task #2007)");
  ...
  // `system_settings` rows the live workers keep re-writing.
  __setZoomCredentialStoreOverrideForTests(credStore);
  try {
    ...
    await runInIsolatedSchema(
      async ({ db }) => {
        await step("manual disconnect → tokens cleared, audit row scoped manual_disconnect", async () => {
          seedTokens("zoom-manual-fake");
          assert.equal(getAccess(), "zoom-manual-fake");
        ...
        await step("terminal-auth disconnect → tokens cleared, audit row scoped connect_terminal_auth_error", async () => {
          ...
          assert.equal(nv?.notes, "Zoom refresh rejected");
        });
  ...
  console.log("\nAll Zoom credential-clear auditing tests passed");
}
