/* test-registration
{
  "name": "SEMrush credential-clear auditing (Task #2007)",
  "regression": true,
  "smoke": true,
...
test-registration */
/**
 * Task #2007 — SEMrush credential-clear auditing.
 *
 ...
 * locks the same invariants in for SEMrush.
 *
 ...
 * and tests/semrush-probe-classification; this test focuses on the
 * disconnect audit that those tests don't exercise:
 *
 ...
 *
 * Task #2240 — credential isolation via in-memory override. SEMrush stores
 * its credential in the shared `system_settings` table, which the live
 * `Start application` SEMrush worker keeps re-writing in the `public`
 * schema while this test runs. Against the shared dev DB that race
 * re-seeds the access token between the disconnect call and the assertion.
 * We now drive the seeded credential (and the device-auth keys) through
 * `__setSemrushCredentialStoreOverrideForTests`: a tiny in-memory `Map`
 * this suite owns end-to-end. `disconnect` clears that map instead of
 * `system_settings`, so the credential assertion never touches a row the
 * live worker also writes. The audit breadcrumb still goes to the DB, so
 * we keep `runInIsolatedSchema` (cloning only `users` +
...
// SEMrush stores its OAuth credential in `system_settings`; the audit
...
// writes its credential + device-auth clears here (never `system_settings`)
// once the override is installed.
...
async function main(): Promise<void> {
  console.log("SEMrush credential-clear auditing (Task #2007)");
  ...
  // `system_settings` rows the live SEMrush worker keeps re-writing.
    ...
    await runInIsolatedSchema(
      async ({ db }) => {
        await step("manual disconnect → tokens cleared, audit row scoped manual_disconnect", async () => {
          seedTokens("semrush-manual-fake");
          assert.equal(getAccess(), "semrush-manual-fake");
        ...
        await step("default trigger (no options) → manual_disconnect audit row", async () => {
          seedTokens("semrush-default-fake");
          const before = (await listClearAudit("manual_disconnect")).length;
          await disconnect();
        ...
        await step("terminal-auth disconnect → tokens cleared, audit row scoped connect_terminal_auth_error", async () => {
          seedTokens("semrush-terminal-fake");
          ...
          assert.equal(nv?.reason, "invalid_grant");
          assert.equal(nv?.notes, "refresh token rejected");
        });
  ...
  console.log("\nAll SEMrush credential-clear auditing tests passed");
}
