/* test-registration
{
  ...
  "regression": true,
  "smoke": true,
  "smokeReason": "Freshness guard for the committed endpoint contract table (audits/D-endpoint-contract-table.{md,json} once rotted ~37 added / 4 removed rows unnoticed, misleading audits). The Validate workflow runs npm run gate, including this lint through gate.ts LINT_CHECKS; this SMOKE_FILES entry adds real-artifact coverage. Fast, DB-free, deterministic (JSON-vs-JSON comparison + tmpdir fixtures).",
  "tier": "small"
}
test-registration */
/**
