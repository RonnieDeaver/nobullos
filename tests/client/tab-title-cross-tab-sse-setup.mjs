// Setup for tests/client/tab-title-cross-tab-sse.test.tsx (Task #3355).
    ...
    // mounts the real CommsProvider graph, which reaches useAuth, so a signed-IN stub lets the REAL use-auth hook fetch
    // /api/auth/user through the suite's fetch stub.
    // Passed via `--import ./tests/client/tab-title-cross-tab-sse-setup.mjs`.
