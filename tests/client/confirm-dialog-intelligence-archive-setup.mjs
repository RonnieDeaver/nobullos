// Setup for tests/client/confirm-dialog-intelligence-archive.test.tsx (Task #4757).
...
// buttons are never queryable. IntelligenceFeed's create/edit Dialog and its
// filter Selects stay closed throughout, so the real @radix-ui/react-dialog /
// react-select stay un-shimmed (which also sidesteps the "shimming dialog
// requires shimming alert-dialog" coupling).
// Passed via `--import ./tests/client/confirm-dialog-intelligence-archive-setup.mjs`.
