...
| [TESTING.md](./TESTING.md) | Test suite and CI control plane — hermetic DB, selection/green evidence, flake quarantine, shards, duration budgets, canary/watchdog roles, pre-deploy gate, emergency override. |
...
| [TASK_SELFCHECK.md](./TASK_SELFCHECK.md) | Per-task self-check checklist — consolidated gate run, TS/lint/test verification, doc/index obligations, migration prefix check, and prior-fix regression check before marking a task done. |
